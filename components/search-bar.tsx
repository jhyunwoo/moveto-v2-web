    ...
    <div className={`relative w-full ${className}`}>
      ...
      >
        ...
        <motion.input
          ...
          className={`z-10 rounded-full border-4 p-2 px-5 transition-colors disabled:animate-pulse disabled:border-sky-400 ${error ? 'border-red-500' : 'border-white'} w-full bg-neutral-900/50 text-xl font-bold outline-hidden transition-colors placeholder:text-xl placeholder:text-gray-300`}
          layoutId={'search'}
          disabled={loading}
          placeholder={'코드 검색'}
          autoComplete={'off'}
