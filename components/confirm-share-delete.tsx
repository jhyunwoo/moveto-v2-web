...
export default function ConfirmShareDelete({ code }: { code: string }) {
  ...
  async function deleteFile() {
    const requestDelete = await fetch(`/api/share/code/${code}`, { method: 'DELETE' })
    const response = await requestDelete.json()
    if (requestDelete.ok) {
      setDeleteShare(false)
      setDeleteError('')
      router.replace('/')
    } else {
