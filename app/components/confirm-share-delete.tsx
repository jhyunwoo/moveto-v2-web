  ...
  return (
    <ModalLayout isOpen={Boolean(deleteShare)} closeModal={closeModal} ariaLabel="공유 삭제 확인">
      <div className="flex items-start gap-3">
        <ExclamationTriangleIcon className="text-danger mt-0.5 size-6 shrink-0" />
        <div>
          <h2 className="font-700 text-lg">공유 파일을 삭제할까요?</h2>
          <p className="text-text-secondary mt-2 text-sm leading-6">
            삭제된 파일과 공유 코드는 다시 복구할 수 없습니다.
          </p>
        </div>
      </div>

      {deleteError ? (
        <p className="bg-danger/10 font-600 text-danger mt-5 rounded-lg px-3 py-2 text-sm" role="alert">
          {deleteError}
        </p>
      ) : null}

      <div className="mt-7 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
        <button type="button" onClick={closeModal} disabled={isDeleting} className="btn-secondary px-5">
          취소
        </button>
        <button
          type="button"
          onClick={deleteFile}
          disabled={isDeleting}
          className="btn-danger px-5 disabled:cursor-wait disabled:opacity-60"
        >
          {isDeleting ? '삭제 중...' : '삭제'}
        </button>
      </div>
